...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/mpctesting/installRosPackages.sh
...
rosrun robot_upstart install mpctesting/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable mpctesting
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
